...
TEST_F(HelperClippingPlanes, ClippingPlanes_half_fit)
{
    ...
    ASSERT_EQ(clipping.currentPoints,
              std::vector<glm::vec2>({glm::vec2(1.01, 1.01),
                                      glm::vec2(1.01, -1.01),
                                      glm::vec2(0, -1.01),
                                      glm::vec2(0, 1.01)}));
}
...
TEST_F(HelperClippingPlanes, ClippingPlanes_polygon_all_in)
{
    ...
    console->debug("points res:\n{}", clipping.toString());
    auto equs = clipping.getEquations();
    console->debug("equ res:\n{}", clippingEquationsToString(equs));
    ASSERT_EQ(equs,
              std::vector<glm::vec3>({
...
TEST_F(HelperClippingPlanes, ClippingPlanes_get_equations)
{

    ClippingPlanes clipping(0.5, 0.25);
